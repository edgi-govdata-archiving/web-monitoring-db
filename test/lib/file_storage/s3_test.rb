...
class FileStorage::S3Test < ActiveSupport::TestCase
  ...
  test 's3 storage can parse S3 URIs' do
    parsed = test_storage.send :parse_s3_url, 's3://test-bucket/some/thing.txt'
  ...
  test 's3 storage can parse S3 URLs' do
    bucket_hostname_url = test_storage.send :parse_s3_url, 'https://test-bucket.s3.amazonaws.com/some/thing.txt'
    ...
    bucket_path_url = test_storage.send :parse_s3_url, 'https://s3.amazonaws.com/test-bucket/some/thing.txt'
    ...
    region_url = test_storage.send :parse_s3_url, 'https://s3-us-west-2.amazonaws.com/test-bucket/some/thing.txt'
  ...
  test 's3 storage will not parse non-S3 URLs' do
    parsed = test_storage.send :parse_s3_url, 'https://google.com/some/thing.txt'
    assert_nil parsed
  end
